...
- **Data:** PostgreSQL (Supabase) là nguồn runtime (xem `server/docs/DATABASE.md`) · `server/data/products.json` chỉ là fixture test/seed · Ảnh: `server/public/images/` (cache 30 ngày).
- **Test:** BE `node:test` (61 test, 6 bộ trong `server/test/`) · UI Playwright trong `tools/` · CI/CD `.github/workflows/` không sửa khi chưa duyệt.
